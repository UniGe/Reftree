...
namespace Ref3.Controllers
{
    ...
    public class InOutFileController : ApiController
    {
        ...
        [HttpGet]
        [Route("ReftreeServiceWorker.js")]
        public IHttpActionResult GetServiceWorker()
        {
            string filePath = HttpContext.Current.Server.MapPath("/Views/3/Js/ReftreeServiceWorker.js");

            if (File.Exists(filePath))
            {
                var response = new HttpResponseMessage();
                response.Content = new StringContent(File.ReadAllText(filePath));
                response.Content.Headers.ContentType = new System.Net.Http.Headers.MediaTypeHeaderValue("application/javascript");

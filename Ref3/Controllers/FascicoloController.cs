...
namespace Ref3.Controllers
{
    ...
    public class FascicoloController : ApiController
    {
        ...
        public HttpResponseMessage GetDossier(DossierRequest data)
        {
            ...
            try
            {
                ...
                foreach (DataRow doc in dbres.table.Rows)
                {
                    ...
                    bool check = fg.AddDocumentToZip(DO_DOCUME_ID, ref zip, ref outfiles, outFilesOfThisDoc, d.DOSSIE_ID, OBJECT_ID);
                    if (check)
                    {
                        package = fg.AddDocumentToExcelList(package, DO_DOCUME_ID, OBJECT_ID, d.DOSSIE_ID);
                ...
                // scarica lo zip
                result = fg.GetChunkedHttpResponseMessage(filename, fname);
        ...
        [HttpPost]
        [RateLimitUserSession(MaxRequest = 6, PerSeconds = 60)]
        public HttpResponseMessage GetDossier_async(DossierRequest data)
        {
            HttpResponseMessage result = MagicFramework.Helpers.Utils.retOkMessage("Dossier loaded");
            try
            {
                ...
                string referenceType = ReferenceTypes.DOSSIER;
                int referenceId = d.DOSSIE_ID;
                ...
                if (FileGenerator.HasProcessingZipJobs(referenceId, userId, dbutils, referenceType))
                {
                    // Return a response indicating that a job is already being processed
                    result = Utils.retOkMessage("Un dossier è già in fase di elaborazione. Si prega di attendere il completamento dell'operazione corrente.");
                    var cookie = new CookieHeaderValue("fileDownload", "true");
                    cookie.Path = "/";
                    cookie.Expires = DateTimeOffset.Now.AddMinutes(1);
                    result.Headers.AddCookies(new[] { cookie });
                    return result;
                }
                ...
                Dictionary<string, string> watermarkedFiles = new Dictionary<string, string>(); // New dictionary for watermarked files

                // Process each document
                foreach (DataRow doc in dbres.table.Rows)
                {
                    List<string> outFilesOfThisDoc = new List<string>();
                    int DO_DOCUME_ID = int.Parse(doc["DO_DOCUME_ID"].ToString());
                    int? OBJECT_ID = null;
                    if (!doc.IsNull("OBJECT_ID"))
                        OBJECT_ID = int.Parse(doc["OBJECT_ID"].ToString());

                    // Add document to queue instead of to zip
                    bool check = fg.AddDocumentToZipQueue(DO_DOCUME_ID, ref outfiles, outFilesOfThisDoc,
                        ref filesToZip, ref pathMapping, ref watermarkedFiles, referenceId, OBJECT_ID);

                    if (check)
                    {
                        package = fg.AddDocumentToExcelList(package, DO_DOCUME_ID, OBJECT_ID, referenceId);
                        if (!documentFiles.ContainsKey(DO_DOCUME_ID))
                            documentFiles.Add(DO_DOCUME_ID, new List<string>());
                        documentFiles[DO_DOCUME_ID] = BL.FileGenerator.getFileNamesFromOutFiles(outFilesOfThisDoc);
                    }
                ...
                // Set up callback URL with proper protocol prefix
                ...
                FileGenerator.TrackZipJob(referenceId, filename, SessionHandler.IdUser, jobId, messageJson, referenceType);
                ...
                // Send message to RabbitMQ directly - this will throw an exception if connection fails
                rabbitMQService.SendMessage(zipMessage);

                // Return a response indicating the process has started
                result = Utils.retOkMessage($"Dossier processing started with job ID: {jobId}. You will be notified when the file is ready for download.");
                // Add the fileDownload cookie to indicate success to the jQuery plugin
                ...
                // Add the job ID as a header for reference
            ...
            {
                // This is for RabbitMQ connection issues or other application-specific errors
                MFLog.LogInFile("Application error in GetDossier: " + ex.Message, MFLog.logtypes.ERROR);
                result = MagicFramework.Helpers.Utils.retInternalServerError("Service unavailable: " + ex.Message);
            }
            catch (Exception ex)
            {
                MFLog.LogInFile("Error in GetDossier: " + ex.Message, MFLog.logtypes.ERROR);
                result = MagicFramework.Helpers.Utils.retInternalServerError(ex.Message);
            }
            return result;
        }

...
def main(argv):
    ...
    if events is not None:
        for event in events.fullChampResults.champResultList:
            for match in event.matches.resultList:
                matches.append(match)
    matches.sort(key=operator.attrgetter("ID"), reverse=False)#kjo esht lista
    s=''
    for x in matches:
        x.teamA.playersList.sort(key=operator.attrgetter("number"), reverse=False)
        x.teamB.playersList.sort(key=operator.attrgetter("number"), reverse=False)
        note=str(x.note).decode('cp1251')
        pos=note.find(' ')
        ...
        filename =str( events.fullChampResults.championshipCode) +str(fileGeneratedCount)+fileExtension

...
function dropDownListOnChange(e) {
    ...
    if (e.sender && e.sender.wrapper) {//dropdownlist case
        elementSelector = e.sender.wrapper;
    } else if (e.selector) {//searchGrid case
...
function prerenderdispatcher(grid, functionname, e, gridselector) {
    ...
    require([window.includesVersion + '/Custom/9/Scripts/config.js'], function () {
    ...
    manageStageConstraints.call(this, e, gridcode, gridentity).then(function (items) {
        $(items).each(function (i, v) {
            ...
            if (v.EV_DEFAULT_VALUE && e.model.isNew()) {
                e.model.set(v.ColumnName, defVal_model_fn(v.EV_DEFAULT_VALUE||v.defaultvalue));
                defvals[v.ColumnName] = { value: defVal_model_fn(v.EV_DEFAULT_VALUE ||v.defaultvalue), editable: v.editable };
            }
        });
        if (origedit__ != null) {
...
function onChangeFieldmanageStageConstraints(e, fname, myGrid, model) {
    //var $grid = $("#" + currentgrid + ",." + currentgrid);

    var $grid = myGrid; //$("tr[data-uid=" + e.container[0].parent().data().uid + "]").closest('div').parent();

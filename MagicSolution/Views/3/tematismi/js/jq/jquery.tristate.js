...
(function($) {
	...
	var pluginName = 'vanderlee.tristate', originalVal = $.fn.val;
	...
	$.widget("vanderlee.tristate",
			{
				...
				_create : function() {
					...
					this.element.click(function(e) {
						...
						case true:
							that.options.state = null;
							break;
						case false:
							that.options.state = true;
							break;
						default:
							that.options.state = false;
							break;
						}
					...
					this.options.checked = this.element.attr('checkedvalue')
							|| this.options.checked;
					this.options.unchecked = this.element
							.attr('uncheckedvalue')
							|| this.options.unchecked;
					this.options.indeterminate = this.element
							.attr('indeterminatevalue')
							|| this.options.indeterminate;

					// Initially, set state based on option state or attributes
					if (typeof this.options.state === 'undefined') {
						this.options.state = typeof this.element
								.attr('indeterminate') !== 'undefined' ? null
				...
				_refresh : function(callback) {
					...
					this.element[this.options.state === null ? 'attr'
							: 'removeAttr']('indeterminate', 'indeterminate');
					this.element.prop('indeterminate',
							this.options.state === null);

					this.element[this.options.state ? 'attr' : 'removeAttr'](
							'checked', true);
					this.element.prop('checked', this.options.state === true);

					if ($.isFunction(callback)) {
						callback.call(this.element, this.options.state, this
								.value());
					}
				},
				...
				value : function(value) {
					if (typeof value === 'undefined') {
						...
						return typeof value === 'undefined' ? this.element
								.attr('value') : value;
					} else {

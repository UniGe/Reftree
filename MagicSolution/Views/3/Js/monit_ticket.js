...
function loadscript() {
    requireConfigAndMore(["MagicMonitor"], function () {
        ...
        initMonitor("Elenco ticket", typeField, "core.TK_USP_GET_MONIT_CLASS", {
            procedureName: "core.TK_USP_GET_MONIT_STACLA",
			filterField: "EV_STACLA_ID",
			entityName: "core.PL_V_ASSET_LIST",
            success: manageFilters,
            isPaging: true
        });
        var gridobj = getrootgrid("MONIT_TICKET");
    ...
    $.each(response.Data[0].Table[0], function (field, v) {
        switch (field) {
            ...
            case 'Totali':
                ...
                break;
            default:
                columns.push({ field: field, filterable: false, title: getObjectText(field.replace(/_/g, " ")), template: '<span class="data-item" data-status="' + field + '">#: ' + field + '  #</span>', width: 100 });
                break;
        }
    });
                    ...
                    $('td.k-state-selected', $row).each(function () {
                        ...
                        if ("status" in data) { // add to statusFilter
                            if (data.status == "all") {
                                ...
                                rowFilter.filters[1].filters.push({ field: "TK_SERVIC_DESCRIPTION", operator: "eq", value: data.status })
                            }
                        }
                    });

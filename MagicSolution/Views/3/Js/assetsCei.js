var treefuncpars = {
    ...
    gridname: "AS_ASSET_CEI",
    entityname: "AS_V_ASSET_assetExtended_CEI",
    pk: "AS_ASSET_ID",
    pkfilter: function (editid) { return { field: "AS_ASSET_ID", operator: "eq", value: editid } },
};

console.log('Entro assetJS');
...
function loadscript() {
    require([window.includesVersion + "/Magic/v/" + window.applicationVersion + "/Scripts/MagicDragAndDropFunctionsUtils.js"],
		function () {
			isGridVisiblePromise(treefuncpars.gridname).then(function () {
				assetLauncher();
			});
        }
    );
...
function assetLauncher() {
	...
	var grid = getrootgrid(treefuncpars.gridname, null, 'grid', null, null);
  ...
  //  grid.dataSource.filter = { field: "AS_ASSET_ID_PADRE", operator: "eq", value: null };
    ...
    kgrid.dataSource.bind("requestEnd", function (e) {
        if (e.type == "create") {
            ...
            if (!response.AS_ASSET_ID_PADRE) {
                ...
                getassettree(window.currentTreeAsset_ID, window.currentTreeTipSas_ID,null,treefuncpars.gridname);
                $(".input-group").show();
            }
...
function assetSelectTree(event) {
    ...
    refreshAssetTabstrip(assetid,treefuncpars.gridname);
}

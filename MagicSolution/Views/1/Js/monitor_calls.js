var typeField = "OutCome_ID";

function loadscript() {
    requireConfigAndMore(["MagicMonitor"], function () {
        apply_style("/Views/1/Styles/monitor.css");  //applica css
        initMonitor("Elenco chiamate", typeField, "Crm.usp_monitor_Calls_Tabs", {
        	procedureName: "Crm.usp_monitor_Calls_Data",
        	filterField: "Campaign_ID",
            success: manageFilters,
            isPaging: true,
            addSearchInput: false
        });
        var gridobj = getrootgrid("Call");
        gridobj.dataSource.filter = { field: typeField, operator: "eq", value: null };
...
function manageFilters(response,opts,filter) {
    ...
    $.each(response.Data[0].Table[0], function (field, v) {
        switch (field) {
            ...
            case 'Campaign_ID':
                return;
            case 'Description':
                columns.push({ field: field, filterable: true, title: "Esito", width: 300, template: '<a class="k-button data-item" onclick="selectCol(event)" >#: Description  #</a>', lockable: false });
                break;
            case 'Totali':
    ...
    $("#gridtotals").kendoGrid({
        ...
        change: function (e) {        	
        	var origfilter = e.sender.dataSource._filter.filters[0];
            ...
            if (selected.length) {
                ...
                $('tr[data-uid]', grid.tbody).each(function () {
                    ...
                    $('td.k-state-selected', $row).each(function () {
                        ...
                        if ("status" in data) { // add to statusFilter                        	
                            if (data.status == "all") {
                            	...
                            	rowFilter.filters[1].filters({ field: "Description", operator: "eq", value: data.status })
                            }
                        }
                    });
                    ...
                    if (hasSelectedCell) {
                    	...
                    	rowFilter.filters.push(origfilter);

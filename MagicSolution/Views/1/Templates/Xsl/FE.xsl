<?xml version="1.0"?>
<xsl:stylesheet xmlns:a="http://ivaservizi.agenziaentrate.gov.it/docs/xsd/fatture/v1.2" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.1">
...
<xsl:when test="$RF='RF03'"> (nuove iniziative produttive) - Non più valido in quanto abrogato dalla legge di stabilità 2015 </xsl:when>
...
<xsl:when test="$NT='N5'"> (regime del margine / IVA non esposta in fattura) </xsl:when>
<xsl:when test="$NT='N6'"> (inversione contabile) </xsl:when>
<xsl:when test="$NT='N7'"> (IVA assolta in altro stato UE) </xsl:when>
...
<xsl:when test="$NAT='N5'"> (regime del margine / IVA non esposta in fattura) </xsl:when>
<xsl:when test="$NAT='N6'"> (inversione contabile) </xsl:when>
<xsl:when test="$NAT='N7'"> (IVA assolta in altro stato UE) </xsl:when>
<xsl:otherwise>
<span>(!!! codice non previsto !!!)</span>
</xsl:otherwise>
</xsl:choose>
...
<xsl:when test="$NAT1='N7'"> (IVA assolta in altro stato UE) </xsl:when>
<xsl:otherwise>
<span>(!!! codice non previsto !!!)</span>
</xsl:otherwise>
</xsl:choose>

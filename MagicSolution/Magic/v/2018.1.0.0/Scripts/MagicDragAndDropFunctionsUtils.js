function renderHiddenGrid(gridobj) {
    ...
    $(".derivativeassetgrid").data("constraint_enabled",'true');
...
function editHiddenGrid(editid) {
    if (!editid) {
		...
		$.when(defer).then(function () {
			...
			var isgrouped = ($(".derivativeassetgrid tr:eq(1)").attr("class") && $(".derivativeassetgrid tr:eq(1)").attr("class").indexOf("k-grouping-row") != -1) ? true : false;
			if (!isgrouped)
				$(".derivativeassetgrid").data("kendoGrid").editRow($(".derivativeassetgrid tr:eq(1)"));
			else
				$(".derivativeassetgrid").data("kendoGrid").editRow($(".derivativeassetgrid tr:eq(2)"));//if it's grouped the 1st is the grouping row. This may occur for example when personal settings are applied to this grid
        });
    }
}

    ...
    bOSelector.prototype = {
        ...
        getGridSelectorDataSource: function () {
            ...
            if (storedDataSource)
                ...
                $.ajax({    //mfapireplaced
                    type: "POST",
                    url: "/api/DocumentRepository/GetBusinessObjectTypes",
                    data: JSON.stringify({ IDs: self.options.filters }),
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
...
function saveBO(tags, callback) {
    ...
    if (BOSdata && BOSdata.BOType && BOSdata.BOId && BOSMessageText !== undefined && BOSMessageType !== undefined) {
        ...
        $.ajax({
            type: "POST",
            aync: true,
            url: "/api/DocumentRepository/GetDocumentRepositoryType", //#mfapireplaced
            data: JSON.stringify({ Description: BOSMessageType }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (result) {
                if (result.Count > 0) {

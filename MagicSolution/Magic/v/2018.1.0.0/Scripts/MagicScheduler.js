   ...
   var resources = [
            ...
            },
            {
				...
				dataSource: {
					transport: {
						read: {
							url: "/api/MAGIC_CALENDAR_TASKSTATUS/GetAvailableLocationsFK",
							type: "GET",
							contentType: "application/json"
						}
					},
    ...
    if (wrkactdd !== undefined) {
        ...
        wrkactdd.bind("cascade", function (e) {
            ...
            if (window.usr_activityselected != "") {
                ...
                $.ajax({    //#mfapireplacede
                    type: "POST",
                    url: "/api/Magic_Calendar_TaskTypes/GetActionTaskTypeByActivityID",
                    data: JSON.stringify({ ActivityID: "" + window.usr_activityselected }),
...
function manageModifications(e) {
	...
	//change address only if i'm in a popUp Bug #5577
	if (window.mapAK && e.container) {
    ...
    $.ajax({
        type: "POST",
        async:false,
        url: manageAsyncCallsUrl(false, url),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: JSON.stringify(data),
        success: function (result) {
...
function saveSchedulerSettings(element)
{
	...
	setConfig("calendar", settings).then(function (res) {
		kendoConsole.log(getObjectText("calendarSettingsSaved"),false);
	});
}
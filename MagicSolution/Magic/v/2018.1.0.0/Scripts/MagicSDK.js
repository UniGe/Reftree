(function (window) {
        ...
        return $.Deferred(function (promise) {
            ...
            if (res.Data && res.Data.length) {
                ...
                $.each(res, function (k, v) {
                    if (!config.storedProcedureKeepEmptyRecordsets) {
                        if (v.drows && v.drows.length && v.drows[0].Table) {
                            result.push(v.drows[0].Table);
                        }
                        else {
                            console.warn('a recordset got ignored', config);
                        }
                    }
                    else {
                        if (v.drows && v.drows.length && v.drows[0].Table) {
    ...
    function removeConfigValues(data) {
        ...
        config.storedProcedureKeepEmptyRecordsets = data.storedProcedureKeepEmptyRecordsets;
        delete data.storedProcedureKeepEmptyRecordsets;
        delete data.$http;
        delete data.$scope;
        return config;
    }
    function kendoGantt_(config)
    {
        ...
        self.magicDateFilter = {
            logic: "AND",
            filters: [
				{ field: "start", operator: "gte", value: minDate },
				{ field: "end", operator: "lte", value: maxDate }],
            type: "dateFilters"
        };
        ...
        self.magicGanttOptions = {
            ...
            edit: function (e) {
                $.ajax({    //#mfapireplaced
                    type: "GET",
                    url: "/api/Magic_Actions/GetCalendarTaskTypes",
                    error: function (err) { console.error(err.responseText) }
                }).then(function (res) {
                    var calendarTypes = res.Data[0].Table;
                    $("#tasktypeIddd").data("kendoDropDownList").setDataSource(calendarTypes);
    ...
    var MF =
    {
        devExtreme: {
            ...
            {
				function buildPivot(options) {

					var afterPivotLoads = function (pivotdata) {
					...
					if (!options.filterFormData && !options.outerGrid)
						return $.getJSON("/api/Magic_Pivot/Get/" + options.code)
							.then(function (pivotdata) {
								return afterPivotLoads(pivotdata);
							});
					else return $.ajax({
						type: 'POST',
						url: "/api/Magic_Pivot/Post/" + options.code,
        ...
        kendo: {
            ...
            getSpreadSheetObject: function (code) {
                ...
                $.ajax({    //#mfapireplaced
                    type: "POST",
                    url: "/api/Magic_Dashboard/GetSpreadsheetByCode",
        ...
        api: {
            get: function (data) {
                ...
                var defaultOptions = {
                    ...
                    //storedProcedureKeepEmptyRecordsets: false, default is false - if true empty recordsets are not removed from stored procedures
                };
                ...
                if (!data.storedprocedure && !data.filter) {
                    console.warn('DEPRECATED usage of GetWithFilter, please pass a "filter" property instead of the "where" property');
                }

                var url = '/api/GENERICSQLCOMMAND/';
				if (data.storedprocedure) {
					delete data.where;
                    url += "SelectDataSetFromXMLStoredProcedure";
                }
                else if (data.filter) {
					url += "Select";
					delete data.where;
					data.EntityName = data.table;
					delete data.table;
                    if (!data.take) {
                        data.take = 1000;
                    }
            ...
            getDataSet: function (options,storeprocedurename,returnEmpty)
            {
				...
				$.when(buildXMLStoredProcedureReturnDataSet(options, storeprocedurename, returnEmpty)).then(function (tables) {
                    data.resolve(tables);
                })
                return data.promise();
            }

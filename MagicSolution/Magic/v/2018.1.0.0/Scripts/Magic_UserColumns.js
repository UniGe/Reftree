function extendUserCols(e) {
    ...
    //get the column list #mfapireplace
    var OnlyForDeveloper = (isDeveloperFunction || window.UserIsDeveloper == "True") ? true : false;
    $.ajax({
        type: "POST",
        url: "/api/Magic_Mmb_Users/GetUserColumnDataTypes",
        data: JSON.stringify({ OnlyForDeveloper: ""+OnlyForDeveloper }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        error: function (err) { console.log(err) }
	}).then(function (res) {
	    var UserColumnDataTypes = res.Data[0].Table;
        defer.resolve(UserColumnDataTypes); //#mfapireplaced
    });
    $.ajax({
        type: "GET",
        url: "/api/Magic_Mmb_Users/GetUserVirtualTables",
        error: function (err) { console.log(err) }
	}).then(function (res) {
		var UserVirtualTables = [];
		if (res.Data.length)
			UserVirtualTables = res.Data[0].Table;
		defer_.resolve(UserVirtualTables); //#mfapireplaced
    });
    $.ajax({
        type: "POST",
        url: "/api/Magic_Mmb_Users/GetBOTypeGridUserTabs",
        data: JSON.stringify({ BoType_ID: rowdata.ID.toString() }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        error: function (err) { console.log(err) }
	}).then(function (res) {
		var BOTypeGridUserTabs = [];
		if (res.Data.length)
			BOTypeGridUserTabs = res.Data[0].Table;
		defer__.resolve(BOTypeGridUserTabs); //#mfapireplaced
    });

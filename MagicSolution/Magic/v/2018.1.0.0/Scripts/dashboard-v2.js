...
function initTabContent($dashboardTabs, templateRequest, title) {
    ...
    if ($dashboardTabContent.length) {
        var helpGUID = $dashboardTabContent.data('helpGuid');
        if (helpGUID) {
            addFunctionHelp(null, helpGUID)
                .then(onClickFn => {
                    var $questionMark = $('<i class="fa fa-question fa-3x" style="padding-right: 15px; display: inline-block; float: right; cursor: pointer;"></i>');
                    $questionMark.click(onClickFn);
                    $dashboardTabContent.prepend($questionMark);
                });
        }
        $dashboardTabContent.addClass('initialized');
        var rangePickerObject = {},
            indicators = {},
            indicatorStored = '',
            charts = {},
            grids = [],
                                ...
                                MF.kendo.getGridObject({
                                    ...
                                    additionalFormDeferrer.then(function () {
                                        ...
                                        gridObject.dataSource.transport.parameterMap = function (options, operation) {
                                            if (operation == "read") {
                                                options.data = JSON.stringify($.extend(JSON.parse(options.data || "{}"), $.extend({}, getTabAdditionalData($dashboardTabContent), {
                                                    _dateFrom: kendo.toString(rangePickerObject.dateStart, "yyyyMMdd"),
                                                    _dateTo: kendo.toString(rangePickerObject.dateEnd, "yyyyMMdd")
                                                })));
                                            }
        ...
        if (indicatorStored || !$.isEmptyObject(charts) || grids.length) {
            rangePickerObject = getGlobalRangePickerObject($dashboardTabContent);

            if (grids.length) {
                rangePickerObject.$button.click(function () {
                    $.each(grids, function (k, $grid) {
...
function initCharts(charts, $tabContent, deferred) {
    ...
    $.each(charts, function (id, chart) {
        var rangePickerObject = getRangePickerObject(),
			obj = {};
		//D.t Feature #5466 modifications in order to mange removal of date selector
		var extension = chart.extension ? JSON.parse(chart.extension) : null;
		...
		if (extension && extension.disableRangePicker) {
			kendo.destroy(rangePickerObject.$rangePicker);
			rangePickerObject.$rangePicker.remove();
		}
		else
...
function getRangePickerObject() {
    ...
    var startyear = dateNow.getMonth() < 2 ? dateNow.getFullYear() - 1 : dateNow.getFullYear();
    var dateEnd = dateNow,
        dateStart = new Date(startyear, 0, 1),
        ...
        from = $('.start', $rangePicker).kendoDatePicker({
            change: function () {
                to.min(this.value())
                rangePickerObject.dateStart = this.value();
            },
            value: dateStart,
            max: dateEnd
        ...
        to = $('.end', $rangePicker).kendoDatePicker({
            change: function () {
                from.max(this.value())
                rangePickerObject.dateEnd = this.value();
            },
            value: dateEnd,
            min: dateStart
        }).data("kendoDatePicker");
...
function getGlobalRangePickerObject($tabContent) {
	...
	if (!$tabContent.data)
		return;
	//var shouldItoggle = $tabContent.data ? !$tabContent.data('hideGlobalRangePicker') : false;
    var rangePickerObject = getRangePickerObject();
    ...
    rangePickerObject.$rangePicker
        ...
        .parent()
		//.toggle(!$tabContent.data('hideGlobalRangePicker'))
		.toggle(!$tabContent.data('hideGlobalRangePicker') )
...
function initChart(chart, data) {
    ...
    switch (chart.chartType) {
            ...
            $chartContainer.data("kendoChart").bind("seriesClick", function (e) {
                if (e.dataItem.customparam && e.dataItem.customparam.functionGuid) {
                    try {
						...
						redirectWithFilter(param.functionGuid, param.gridName, param.functionId, param.filter, param.queryString);
                    }
                    catch (err) {
                        console.log(err);
                    }
        ...
        case "kendolinechart":
			...
			var labels = [],
			    ...
			    isstacked = partialLabels.length > 1 ,//This means that it has more than 1 value for the same X value
                series = $.map(isstacked ? data[0].Partials : [0], function (v, k) {
                    return {
                        data: [],
                        markers: { type: "square" },
						color: param["colorPartial" + (k + 1)],
						name: isstacked ? (param["legendPartial" + (k + 1)] || partialLabels[k] || "P" + (k + 1)) : null
                    };
				});
    ...
    if (!$('.btn-group.pull-right', chart.$element).length) {
        $('.rangePicker', chart.$element).before(getExportButtons({
            ...
            style: "margin-left: 15px;margin-top:14px;margin-right:2px;"
        }, chart.description));
    }
}
...
function redirectWithFilter(funcGuid, gridName, funcId, filter,queryString) {
	...
	redirectToFunction(funcGuid, queryString);
}
...
function addUserCustomization(tab) {
    ...
    requireConfigAndMore(["MagicSDK"], function (MF) {
        ...
        $.ajax({ //#mfapireplaced
            type: "GET",
            url: "/api/Magic_Dashboard/GetDashboardContentType",
            error: function (err) {
                console.log(err);
                defer1.resolve([]);
            }
        }).then(function (res) {
            var data = res.Data[0].Table[0];
            var contentTypes = $.map(data, function (v, i) { return $.extend(v, { icon: iconsToShow[v.Code] }); });

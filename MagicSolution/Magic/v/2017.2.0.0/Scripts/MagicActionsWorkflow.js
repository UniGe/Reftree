        ...
        MF.api.get({
            ...
            MF.api.get({ storedProcedureName: "dbo.WKF_get_outstatus", taskId: taskid, culture: window.culture.substring(0, 2) }).then(function (result) {
                ...
                var config = {
                    ...
                    isFileRequired: wact.fileRequired,
                    isSingleUser: wact.singleUser,
...
function taskTimeSheet(taskid, refreshCallBack)
{
    requireConfigAndMore(["MagicSDK"], function (MF) {
        MF.api.get({
            ...
            //    data: JSON.stringify({ table: "dbo.v_Magic_Cal_Wkf_OutStatuses_L", order: "5", where: "taskId = " + taskid + " AND Language = '" + window.culture.substring(0, 2) + "'" }),
...
function assign(taskid, refreshCallBack) {
    var minDate,
        duedate,
        assigneeUserID,
        userInputType = window.workflowactions_rowdata.singleUser ? "kendoDropDownList" : "kendoMultiSelect",
        schema = {
            type: "object",
            format: "grid",
            properties: {
    ...
    var savefn = function () {
        ...
        var postcontent = buildGenericPostInsertUpdateParameter("update", "dbo.Magic_Calendar", "taskId", assign_task_SP, "XML", -1, -1, { taskId: taskid, assigneeUserID: users.join(), dueDate: duedate ? toTimeZoneLessString(duedate) : duedate, duration: duration, red_at: window.workflowactions_rowdata.red_at });
        $.ajax({
            type: "POST",

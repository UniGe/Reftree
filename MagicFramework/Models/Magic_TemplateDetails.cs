using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;

namespace MagicFramework.Models {

public class Magic_TemplateDetails{

public int MagicTemplateDetailID {get;set;}
public int MagicTemplate_ID {get;set;}
public int MagicTemplateGroup_ID {get;set;}
public int OrdinalPosition {get;set;}
public int? MagicDataRole_ID {get;set;}
public string MagicNullOptionLabel {get;set;}
public string MagicDataSourceValueField {get;set;}
public string MagicDataSourceTextField {get;set;}
public string MagicDataSource {get;set;}
public bool? Detailisvisible {get;set;}
public int? DetailInheritsFromColumn_ID {get;set;}
public string DetailonchangeFunctionName {get;set;}
public string DetailDOMID {get;set;}
public int DetailInheritsFromGroup_ID { get; set; }
public int MagicDataSourceType_ID { get; set; }
public string MagicDataSourceSchema { get; set; }
public int? SearchGrid_ID { get; set; }
public int? SearchGridDescColumn_ID { get; set; }
public int? CascadeColumn_ID { get; set; }
public int? CascadeFilterCol_ID { get; set; }

public Magic_TemplateDetails(MagicFramework.Data.Magic_TemplateDetails A) {
this.MagicTemplateDetailID = A.MagicTemplateDetailID;
this.MagicTemplate_ID = A.MagicTemplate_ID;
this.MagicTemplateGroup_ID = A.MagicTemplateGroup_ID;
this.OrdinalPosition = A.OrdinalPosition;
this.MagicDataRole_ID = (int)(A.MagicDataRole_ID ?? 0);
this.MagicNullOptionLabel = A.MagicNullOptionLabel;
this.MagicDataSourceValueField = A.MagicDataSourceValueField;
this.MagicDataSourceTextField = A.MagicDataSourceTextField;
this.MagicDataSource = A.MagicDataSource;
this.Detailisvisible = A.Detailisvisible;
this.DetailInheritsFromColumn_ID = (int)(A.DetailInheritsFromColumn_ID ?? 0);
this.DetailonchangeFunctionName = A.DetailonchangeFunctionName;
this.DetailDOMID = A.DetailDOMID;
this.DetailInheritsFromGroup_ID = (int)(A.DetailInheritsFromGroup_ID ?? 0);
this.MagicDataSourceType_ID = (int)(A.MagicDataSourceType_ID ?? 0);
this.MagicDataSourceSchema = A.MagicDataSourceSchema;
this.SearchGrid_ID = (int)(A.SearchGrid_ID ?? 0);
this.SearchGridDescColumn_ID = (int)(A.SearchGridDescColumn_ID ?? 0);
this.CascadeColumn_ID = (int)(A.CascadeColumn_ID ?? 0);
this.CascadeFilterCol_ID = (int)(A.CascadeFilterCol_ID ?? 0);

}
public Magic_TemplateDetails()
{ }
public  string getHtmlTemplate(Data.Magic_TemplateDetails detail,string label,string appendextraattribute,string line,bool popup = true,bool inLine = false,bool inCell = false)
{

    var tcb = new Helpers.TemplateContainerBuilder(true);
    tcb.PrepareDataRoles();


    string searchgridname = null;
    string searchgriddesccol = null;

    if (detail.Magic_Grids != null)
    {
        searchgridname = detail.Magic_Grids.MagicGridName;
        if (detail.Magic_Columns1 != null)
            searchgriddesccol = detail.Magic_Columns1.ColumnName;
    }

    string cascadeFiltercol = null;
    string cascadecol = null;
    string operatorforCascadefilter = null; //caso stringa (Schema_type == null)
    if (detail.Magic_Columns2 != null)
    {
        operatorforCascadefilter = "contains"; //caso stringa (Schema_type == null)
        if (detail.Magic_Columns2.Schema_type != null)  // se non e' una stringa la colonna cascade
            operatorforCascadefilter = "eq";
        cascadecol = detail.Magic_Columns2.ColumnName;
        if (detail.Magic_Columns3 != null)
            cascadeFiltercol = detail.Magic_Columns3.ColumnName;
    }
    if (detail.Magic_Columns.Schema_required != null)
        if ((bool)detail.Magic_Columns.Schema_required)
            appendextraattribute += " required";
    if (detail.Magic_TemplateDataRoles.MagicTemplateDataRole == "searchgrid_autocomplete" && string.IsNullOrEmpty(detail.MagicDataSource))
        appendextraattribute += " data-bind=\"value:" + detail.MagicDataSourceTextField + "\"";

    string datarolefile = detail.Magic_TemplateDataRoles.MagicTemplateDataRole + (inCell ? "_incell" : "");

    line += String.Format(tcb.GetTemplateContent(datarolefile), detail.Magic_Columns.ColumnName, label, detail.MagicDataSourceTextField, searchgridname, searchgriddesccol, cascadecol, cascadeFiltercol, operatorforCascadefilter, appendextraattribute, detail.MagicDataSourceValueField, detail.Magic_Grids.FromTable);
    return line;

}

}
}
